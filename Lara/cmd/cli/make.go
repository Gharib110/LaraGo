...
func doMake(arg2, arg3, arg4 string) error {
	...
	case "migration":
		...
		err := la.CreatePopMigration([]byte(up), []byte(down), arg3, migrationType)
		if err != nil {
			exitGracefully(err)
		}
	...
	case "handler":
		...
		fileName := la.RootPath + "/handlers/" + strings.ToLower(arg3) + ".go"
	...
	case "model":
		...
		fileName := la.RootPath + "/data/" + strings.ToLower(modelName) + ".go"
		if fileExists(fileName) {
			exitGracefully(errors.New(fileName + " already exists!"))
		}
	...
	case "mail":
		...
		htmlMail := la.RootPath + "/mail/" + strings.ToLower(arg3) + ".html.tmpl"
		plainMail := la.RootPath + "/mail/" + strings.ToLower(arg3) + ".plain.tmpl"
